...
def findClosest(points, point, axis=-1):
    '''
    axis 0 -> X
    axis 1 -> Y
    '''
    ...
    if(axis==-1): #Check wether an axis was specified
        rating = dist
    else:         #Assign rating as a weighted sum of distance and axis coordinate 
        rating = dist + abs(((points[:,axis])**2)/np.amax(points[:,axis]))
    
    return (points[np.where(rating == np.amin(rating))] + point) #Return elements wich correspond with the index of the
